...
"""
This module defines the ArgparseCompleter class which provides argparse-based tab completion to thgcmd apps.
...
from . import ansi, thgcmd, constants
...
class ArgparseCompleter:
    ...
    def __init__(self, parser: argparse.ArgumentParser, thgcmd_app: thgcmd.Cmd, *,
                 parent_tokens: Optional[Dict[str, List[str]]] = None) -> None:
        """
        Create an ArgparseCompleter
        :param parser: ArgumentParser instance
        :param thgcmd_app: reference to the thgcmd application that owns this ArgparseCompleter
    ...
    def complete_command(self, tokens: List[str], text: str, line: str, begidx: int, endidx: int, *,
                    ...
                    if remaining_positionals:
                        ...
                        if action == self._subcommand_action:
                            if token in self._subcommand_action.choices:
                                ...
                                completer = ArgparseCompleter(self._subcommand_action.choices[token], self._thgcmd_app,
        ...
        if flag_arg_state is not None:
            ...
            if completion_results:
                ...
                if not self._thgcmd_app.completion_hint:
                    self._thgcmd_app.completion_hint = _build_hint(self._parser, flag_arg_state.action)
        ...
        # Otherwise check if we have a positional to complete
            ...
            if completion_results:
                # Don't overwrite an existing hint
                if not self._thgcmd_app.completion_hint:
                    self._thgcmd_app.completion_hint = _build_hint(self._parser, pos_arg_state.action)
                return completion_results
    ...
    def _complete_flags(self, text: str, line: str, begidx: int, endidx: int, matched_flags: List[str]) -> List[str]:
        ...
        for action, option_strings in matched_actions.items():
            ...
            if not action.required:
                flag_text = '[' + flag_text + ']'
            self._thgcmd_app.display_matches.append(flag_text)
    ...
    def _format_completions(self, arg_state: _ArgumentState, completions: List[Union[str, CompletionItem]]) -> List[str]:
        ...
        if 1 < len(completions) <= self._thgcmd_app.max_completion_items and \
            ...
            if not self._thgcmd_app.matches_sorted:
                completions.sort(key=self._thgcmd_app.default_sort_key)
            ...
            self._thgcmd_app.completion_header = hint_table.generate_header()
            self._thgcmd_app.display_matches = [hint_table.generate_data_row([item, item.description]) for item in completions]

        return completions

    def complete_subcommand_help(self, tokens: List[str], text: str, line: str, begidx: int, endidx: int) -> List[str]:
        """
        Supports thgcmd's help command in the completion of subcommand names
    ...
    def format_help(self, tokens: List[str]) -> str:
        """
        Supports thgcmd's help command in the retrieval of help text
        :param tokens: command line tokens
    ...
    def _complete_for_arg(self, arg_state: _ArgumentState,
        ...
        if isinstance(arg_choices, ChoicesCallable):
            if arg_choices.is_method:
                ...
                cmd_set = self._thgcmd_app._resolve_func_self(arg_choices.to_call, cmd_set)
                if cmd_set is None:
        ...
        if not results:
            ...
            self._thgcmd_app.matches_sorted = False
            return []
