...
It also defines a parser class called thgcmdArgumentParser which improves error
and help output over normal argparse. All thgcmd code uses this parser and it is
recommended that developers of thgcmd-based apps either use it or write their own
parser that inherits from it. This will give a consistent look-and-feel between
the help/error output of built-in thgcmd commands and the app-specific commands.
If you wish to override the parser used by thgcmd's built-in commands, see
override_parser.py example.
Since the new capabilities are added by patching at the argparse API level,
they are available whether or not thgcmdArgumentParser is used. However, the help
and error output of thgcmdArgumentParser is customized to notate nargs ranges
...
**Tab Completion**
thgcmd uses its ArgparseCompleter class to enable argparse-based tab completion
...
needs to be an instance method of a thgcmd.Cmd or thgcmd.CommandSet subclass. When
ArgparseCompleter calls the method, it well detect whether is is bound to a
CommandSet or Cmd subclass.
If bound to a thgcmd.Cmd subclass, it will pass the app instance as the `self`
argument. This is good in cases where the list of choices being generated
relies on state data of the thgcmd-based app.
If bound to a thgcmd.CommandSet subclass, it will pass the CommandSet instance
...
thgcmd's instance variables related to tab completion, it will be rare to need a
...
thgcmd provides a few completer methods for convenience (e.g.,
...
that will be shown. Its value is defined in thgcmd.Cmd.max_completion_items. It
...
############################################################################################################
# The following are names of custom argparse argument attributes added by thgcmd
############################################################################################################
...
def _SubParsersAction_remove_parser(self, name: str):
    ...
    class so thgcmd can remove subcommands from a parser.
...
class thgcmdHelpFormatter(argparse.RawTextHelpFormatter):
...
class thgcmdArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self,
                 ...
                 epilog=None,
                 parents=None,
                 formatter_class=thgcmdHelpFormatter,
                 prefix_chars='-',
        ...
        super(thgcmdArgumentParser, self).__init__(
            prog=prog,
            usage=usage,
...
class thgcmdAttributeWrapper:
    """
    Wraps a thgcmd-specific attribute added to an argparse Namespace.
    This makes it easy to know which attributes in a Namespace are
    arguments from a parser and which were added by thgcmd.
    """
...
# The default ArgumentParser class for a thgcmd app
DEFAULT_ARGUMENT_PARSER = thgcmdArgumentParser


def set_default_argument_parser(parser: Type[argparse.ArgumentParser]) -> None:
    """Set the default ArgumentParser class for a thgcmd app"""
